"""empty message

Revision ID: 880a970f0d8c
Revises: 
Create Date: 2020-03-12 22:15:38.266452

"""
from alembic import op
import sqlalchemy as sa


# revision identifiers, used by Alembic.
revision = '880a970f0d8c'
down_revision = None
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_table('employee',
    sa.Column('eid', sa.Integer(), nullable=False),
    sa.Column('ename', sa.String(length=64), nullable=True),
    sa.Column('email', sa.String(length=120), nullable=True),
    sa.Column('password_hash', sa.String(length=128), nullable=True),
    sa.PrimaryKeyConstraint('eid')
    )
    op.create_index(op.f('ix_employee_email'), 'employee', ['email'], unique=True)
    op.create_index(op.f('ix_employee_ename'), 'employee', ['ename'], unique=True)
    op.create_table('items',
    sa.Column('itemid', sa.Integer(), nullable=False),
    sa.Column('itemname', sa.String(length=64), nullable=True),
    sa.Column('price', sa.Integer(), nullable=True),
    sa.Column('item_nature', sa.String(length=64), nullable=True),
    sa.PrimaryKeyConstraint('itemid')
    )
    op.create_index(op.f('ix_items_itemname'), 'items', ['itemname'], unique=True)
    op.create_table('order',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('item_id', sa.Integer(), nullable=True),
    sa.Column('e_id', sa.Integer(), nullable=True),
    sa.Column('quantity', sa.Integer(), nullable=True),
    sa.Column('order_date', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['e_id'], ['items.itemid'], ),
    sa.ForeignKeyConstraint(['item_id'], ['employee.eid'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_order_order_date'), 'order', ['order_date'], unique=False)
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_index(op.f('ix_order_order_date'), table_name='order')
    op.drop_table('order')
    op.drop_index(op.f('ix_items_itemname'), table_name='items')
    op.drop_table('items')
    op.drop_index(op.f('ix_employee_ename'), table_name='employee')
    op.drop_index(op.f('ix_employee_email'), table_name='employee')
    op.drop_table('employee')
    # ### end Alembic commands ###
